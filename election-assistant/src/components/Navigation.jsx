...
export default function Navigation() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-civic-blue/95 backdrop-blur-md shadow-sm border-b border-white/5" aria-label="Main navigation">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <NavLink to="/" className="flex items-center gap-2.5 group">
            <div className="w-8 h-8 rounded-lg bg-civic-gold flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform">
          ...
          <button
            ...
            type="button"
            onClick={() => setOpen((v) => !v)}
            className="lg:hidden inline-flex items-center justify-center p-2 rounded-lg text-blue-200 hover:text-white hover:bg-white/10 transition-colors"
            aria-expanded={open}
      ...
      {open && (
        <div id="mobile-menu" className="lg:hidden animate-slide-down bg-civic-blue border-t border-white/10">
          <div className="px-4 py-3 space-y-1">
            {links.map((l) => (
              ...
              >
                {l.label}
                {l.to === "/quiz" && !quizState.completed && (
                  <span className="ml-2 inline-flex h-2 w-2 rounded-full bg-civic-gold" />
                )}
              </NavLink>
            ))}
          </div>
